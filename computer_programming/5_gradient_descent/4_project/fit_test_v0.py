...
#      just knowing the data points x,y and
#      the general shape of the function s.
...
# the model, how good is the cost function
# for your particular situation, etc.).
...
param_true = [a_true, b_true, c_true]
...
a_guess, b_guess, c_guess = 2.1, 2.0, 1.2
param_guess = [a_guess, b_guess, c_guess]
...
print(f"MSE of the guess: {discrepancy(a_guess, b_guess, c_guess, x_data, y_data)}")
print(f"MSE of the true model: {discrepancy(a_true, b_true, c_true, x_data, y_data)}")
...
param_gd, param_gd_traj, converged = grad_desc(
    lambda p: MSE(p, x_data, y_data), np.array(param_guess), alpha=0.01, max_epochs=1000
)
print(f"MSE of the GD guess: {MSE(param_gd, x_data, y_data)}")

y_gd = s(x_grid, *param_gd)
plt.plot(x_grid, y_gd, c="orange")


res = minimize(MSE, param_guess, args=(x_data, y_data))
param_min = res.x
print(f"MSE of the scipy guess: {MSE(param_min, x_data, y_data)}")

y_min = s(x_grid, *param_min)
plt.plot(x_grid, y_min, c="green")
plt.show()
