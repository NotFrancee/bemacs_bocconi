...
def get_seam(grad):
    ...
    c[0] = grad[0]
    # make it a matrix and apply argmin along an axis
    cs = np.zeros(3)  # will store cl, ct, cr

    for i in range(1, n):  # this cannot be vectorized
        for j in range(m):  # this can be
            cl = c[i - 1, j - 1] if j != 0 else np.infty
            ct = c[i - 1, j]
            cr = c[i - 1, j + 1] if j != m - 1 else np.infty

            cs[0], cs[1], c[2] = cl, ct, cr

            opt_choice = np.argmin(cs)
            opt_cost = cs[opt_choice]
